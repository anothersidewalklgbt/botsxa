...
# ---------- PostgreSQL ----------
DATABASE_URL = os.getenv("DATABASE_URL")  # Railway expone esta variable automáticamente
...
def save_account(username, password):
    try:
        ...
        conn.commit()
        print(f"✅ Guardado en DB: {username}")
    except Exception as e:
        print(f"⚠️ Error guardando en DB para {username}: {e}")
        conn.rollback()


def read_existing_users_from_db():
...
def main():
    ...
    existing_users = read_existing_users_from_db()
            ...
            try:
                ...
                for sel in signup_selectors:
                    try:
                        ...
                        if button:
                            page.click(sel)
                            print(f"[{idx}] Click en botón de registro ({sel})")
                            clicked = True
                            break
                    except Exception:
                        continue
                ...
                if not clicked:
                    print(f"[{idx}] error: no se pudo clicar el botón de registro")
                    context.close()
                    time.sleep(PAUSE_BETWEEN)
                    continue
                ...
                if success:
                    save_account(user, pwd)
                    created_this_run.add(user)
                    created_ordered.append((user, pwd))
                    if user != original_user:
                        print(f"[{idx}] '{original_user}' ya existía → usando '{user}'. Guardado en DB.")
                    else:
                        print(f"[{idx}] creado: {user} (guardado en DB)")
                else:
                    print(f"[{idx}] posible fallo al crear {user}.")
...
if __name__ == "__main__":
    if not TARGET:
        print("ATENCIÓN: no has configurado TARGET.")
    main()
